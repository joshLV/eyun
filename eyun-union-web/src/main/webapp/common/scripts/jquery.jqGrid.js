	...
	styleUI : {
		jQueryUI : {
			...
			base : {
				...
				viewBox : "", // view diw
				headerTable : "",
				headerBox : "ui-state-default tab_bg_header", // 表格头部式样修改，默认使用系统定制头部式样 upd by baox
				rowTable : "",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			height: 150,
			page: 1,
			rowNum: 10,   /** 默认每页显示行数为20，根据项目，修改为10 update by baox **/
			rowTotal : null,
			records: 0,
			pager: "",
			pgbuttons: true,
			...
			hoverrows : true,
			altclass : 'tab_bg_odd odd', // 表格换行变色默认式样修改，odd式样无实际意义，只是为了判断是否为奇数行，原默认class为：“ui-priority-secondary” upd by baox
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			result += (clas !== undefined ? (" class=\"cell_border_clear "+clas+"\"") :" class=cell_border_clear") + ((cm.title && tv) ? (" title=\""+$.jgrid.stripHtml(tv)+"\"") :""); // 去除表格边框样式添加 upd by baox
			...
			if(!$t.p.multiselect) {
				if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow !== pt.id ) {
						...
						if( csr ) {
							$(  csr ).removeClass(highlight).attr({"aria-selected":"false", "tabindex" : "-1"});if($(  csr ).hasClass("odd"))$(  csr ).addClass("tab_bg_odd"); // 行取消选中时，奇数行添加自定义class upd by baox
						}
						$(pt).addClass(highlight).removeClass("tab_bg_odd").attr({"aria-selected":"true", "tabindex" : "0"});//.focus(); // 行选中时，奇数行自定义class移除，否则点击后背景颜色不会变 upd by baox
						if(fid) {
				...
				if (  ia === -1 ){
					if(pt.className !== "ui-subgrid") { $(pt).addClass(highlight).removeClass("tab_bg_odd").attr("aria-selected","true");} // 行选中时，奇数行自定义class移除，否则点击后背景颜色不会变 upd by baox
					stat = true;
					$t.p.selarrrow.push($t.p.selrow);
				} else {
					if(pt.className !== "ui-subgrid") { $(pt).removeClass(highlight).attr("aria-selected","false");if($(pt).hasClass("odd")){$(pt).addClass("tab_bg_odd");}} // 行取消选中时，奇数行添加自定义class upd by baox
					stat = false;
			...
			if(shrink===true) {
				...
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						...
						// dont'mod it if you can't figure out
						if(isNaN(cw)){
							cw =-1;//解决ie8 强制block
						}
						// end

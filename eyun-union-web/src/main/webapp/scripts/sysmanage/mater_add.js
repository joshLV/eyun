 ...
 *//**开始上传图片**//*
 ...
 url: ctx + '/systemManage/material/saveMaterial.do?type=' + type+'&materialName='+'1111',
            secureuri: false,
 ...
 *//*console.log(data.url);
 ...
 $(".uploadMess").removeClass("clr-error").html("保存成功");
 // saveOrUpdMaterial(ctx);
                } else {
                    $("#cmaterialUrl").val("");
 $(".uploadMess").addClass("clr-error").html("保存失败");
...
/*function saveOrUpdMaterial(ctx) {
 ...
 /*//*判断是否有上传图片的url*
    ...
    //请求地址
    var url = ctx + '/systemManage/material/saveMaterial.do';
    if ($.trim(materialId)) {
        url = ctx + '/systemManage/material/updMaterial.do';
    }
    $.ajax({
        async: false,
        cache: true,
...
function updateMaterialStatus(ctx) {
    ...
    $.ajax({
        ...
        cache: true,
        type: "POST",
        url: ctx + '/systemManage/material/submitMaterial.do',

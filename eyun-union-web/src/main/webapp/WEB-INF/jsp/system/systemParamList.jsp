...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	...
	<div id="systemParamDialog" style="display : none;">
		<table class="PublicTable">
			...
			<tr id="protocolTr">
				<td>协议：</td>
				<td><textarea id="protocol" cols="45" rows="5" class="PublicText"></textarea></td>
			</tr>
			<tr id="discountInfoTr">
				<td>打折信息：</td>
				<td><textarea id="discountInfo" cols="45" rows="5" class="PublicText"></textarea></td>
			</tr>
			<tr>
				<td>进店提醒：</td>
				<td><lable>
			...
			<tr>
				<td style="display : none;" colspan="2"><input type="reset" id="resetButton" /> 
				<input type="hidden" id="placeID" /> 
				<input type="hidden" id="ctrlCode" /></td>
			</tr>
		</table>
	</div>
...
<script type="text/javascript">
	$(function(){
		$("#gridTable").jqGrid({
			url:'${ctx}/system/systemParam/loadGrid.do',
			mtype : 'post',
			datatype : "json",
			...
			altRows : true,
			colNames:['场所名称','是否允许加入会员','是否开启进店提醒','所属账户', '操作', 'id', 'placeId', 'placeCode', 'ctrlCode', 'protocol', 'discountInfo'],
			colModel:[
				...
				{name:'accountResource',index:'accountResource',sorttype:"string", align: 'center', width : '10'},
			...
			gridComplete:function(rowid, rowdata){
				...
				for(var i=0;i<ids.length;i++){
					...
					for(var i=0;i<rows.length;i++){
						if(rows[i].accountResource=="1"){
							jQuery('#gridTable').jqGrid('setRowData',rows[i].id,{accountResource:'本账号'});
						}
						if(rows[i].accountResource=="0"){
							jQuery('#gridTable').jqGrid('setRowData',rows[i].id,{accountResource:'子账号'});						
						}
					}
				//异步加载js，确定拥有的权限
	...
	function updateSystemSet() {
		...
		showMsg("确定修改系统设置吗？", 2, function() {
			$(this).dialog('close');
			$.ajax({
				url : '${ctx}/system/systemParam/updSysParam.do',
				type : 'post',
				dataType : 'json',

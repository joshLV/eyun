	...
	<div id="OperatorDialog" style="display : none;">
		<form>
			<table class="PublicTable">
				...
				<tr>
					<td>登陆密码：<span id="pswSpan">*</span></td>
					<td><input type="password" id="psw" class="PublicText"/></td>
				
					<td>确认密码：<span id="surePswSpan">*</span></td>
				...
				<tr>
					<td>场所：<span>*</span></td>
					<td colspan="3" id="areaId">
						<c:if test="${not empty creatorRole}">
							<c:if test="${creatorRole.id != 1}">
								<c:forEach var="entity" items="${places}" varStatus="status">
									<c:if test="${not empty entity}">
										<c:if test="${status.count%4==0}"></br></c:if>
										<label><input type="checkbox" name="place" value="${entity.code }"/>${entity.name }</label>
									</c:if>
								</c:forEach>
							</c:if>
						</c:if>
						<c:if test="${empty creatorRole}">
...
<script type="text/javascript">
	...
	function updateOperator(id) {
		...
		if(${not empty creatorRole} && ${creatorRole.id == 1}) {
			...
			if(placeStr != null && placeStr != "") {
				var placeArray = placeStr.split(",");
				for(var i = 0; i < placeArray.length; i++) {
					$("input[name='place'][value=" + placeArray[i] + "]").prop("checked", true);
				}
			}
		}
		$("#pswSpan").hide();
	...
	function addOrUpdateOperateUser(){
		...
		if(id != null && id != "") {
			if((psw != null && psw != "") || (surePsw != null && surePsw != "")) {
				if(psw == null || psw == "") {
					showMsg("请输入登陆密码！");
					return;
				}
		...
		showMsg(msg, 2, function() {
			...
			$.ajax({
				...
				type : "post",
				dataType : "json",
				data : {"id" : id, "memberRealName" : memberRealName, "memberName" : memberName, "memberPwd" : psw, "roleID" : roleID, "remark" : remark, "status" : status, "searchValue" : placeStr},

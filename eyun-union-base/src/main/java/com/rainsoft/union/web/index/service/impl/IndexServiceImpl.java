package com.rainsoft.union.web.index.service.impl;
...
//			result.setMessage("查无数据！");
			result.setMessage(requestContext.getMessage("errors.noData"));
...
//			}
//			if (userInfo.getSmsBalance() == null || "".equals(userInfo.getSmsBalance())) {
//				userInfo.setSmsBalance("0");
//			}
			if (userInfo.getMemberHP() == null || "".equals(userInfo.getMemberHP())) {
				userInfo.setMemberHP(map.get("contextPath") + "/images/1111.jpg");
			}
			JSONObject obj = (JSONObject) JSONObject.toJSON(userInfo);
			result.setData(obj);
			//result.setMessage("查询成功！");
			result.setMessage(requestContext.getMessage("info.find.success"));
			result.setStatus(Constants.RETURN_SUCCESS);
		} else {
			userInfo = new Index();
//			if (userInfo.getWwbBalance() == null || "".equals(userInfo.getWwbBalance())) {
...
//			result.setMessage("查询失败！");
			result.setMessage(requestContext.getMessage("errors.find.fail"));
			result.setStatus(Constants.RETURN_ERROR);
			result.setData(obj);
		}
		return result;
	...
	/*public Result findVisitorNumByMemberid(Integer memberId) {
		Result result = new Result();
		Index index = indexDao.findBy("findVisitorNumByMemberid", memberId);
		String[] arr = index.getContent().split(",");
		JSONArray jsonArr = new JSONArray();
		for(String str : arr) {
			JSONObject jsObj = new JSONObject();
			jsObj.put("name", str.substring(0, str.lastIndexOf("_")));
			jsObj.put("y", Integer.parseInt(str.substring(str.lastIndexOf("_") + 1)));
			jsonArr.add(jsObj);
		}

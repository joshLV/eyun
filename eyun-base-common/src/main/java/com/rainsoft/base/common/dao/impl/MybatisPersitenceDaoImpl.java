package com.rainsoft.base.common.dao.impl;
...
import com.rainsoft.base.common.dao.SQLAdapter;
import com.rainsoft.base.common.model.ConditionFilter;
import com.rainsoft.base.common.model.IdEntity;
import com.rainsoft.base.common.utils.ReflectGenricUtil;
import com.rainsoft.base.common.utils.StringUtil;

@SuppressWarnings("unchecked")
public abstract class MybatisPersitenceDaoImpl<T extends IdEntity, ID extends Serializable> extends SqlSessionDaoSupport implements IMybatisPersitenceDao<T, ID> {
	...
	private static final String SEARCH ="FILTERS";
	...
	/**
	 * 功能说明：获取默认SqlMapping命名空间 <br/>
	 * 使用泛型参数中业务实体类型的全限定名作为默认的命名空间。 <br/>
	...
	/**
	 * 功能说明：设置SqlMapping命名空间。 此方法只用于注入SqlMapping命名空间，以改变默认的SqlMapping命名空间，
	...
	public List<T> findListBy(T item, String sortColumn, String des) {
		...
		try {
			...
			if (item != null) {
				...
				paramMap = BeanMapUtil.toMap(item);
				String searchJson = item.getSearchValue();
				if (StringUtil.isNotEmpty(searchJson)) {
					// 查询字段不为空，过滤其中可能存在的非法字符
					paramMap.put(SEARCH, getFiltersByJson(searchJson));
	...
	public List<T> findListBy(String keyId, T item, String sortColumn, String des) {
		...
		return this.getSqlSession().selectList(getSqlName(keyId), paramMap);
	 ...
	 * @return
	 */
	protected List<ConditionFilter> getFiltersByJson(String json){
		List<ConditionFilter> cfList = new ArrayList<ConditionFilter>();
		JSONArray jsonArr = JSONArray.parseArray(json);
		for(int i=0;i<jsonArr.size();i++){
			JSONObject jsonObj =  (JSONObject)jsonArr.get(i);
			cfList.add(JSON.toJavaObject(jsonObj, ConditionFilter.class));
        }  
		return cfList;
	}
	 ...
	 * @throws
	 * @author yty
	 * @date 2016年5月3日下午4:01:02
	 */
	public Integer findCountByKeyId(String keyId, T item)throws SQLException {

		return this.getSqlSession().selectOne(getSqlName(keyId), item);
	 ...
	 * @throws
	 * @author yty
	 * @date 2016年5月5日下午6:30:30
	 */
	public Integer findCountByKeyId(String keyId, Object... obj)throws SQLException {
		return this.getSqlSession().selectOne(getSqlName(keyId), obj);
	}
